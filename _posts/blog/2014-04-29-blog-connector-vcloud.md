---
layout: article
title: How to Configure Your VMWare vCloud SlipStream Connector
category: blog
...
value of the configuration, as described in the section above. Here is a screenshot of the parameter to
define:
...
you to configure how the connector should communicate with the IaaS cloud endpoint.
...
But most of time this value corresponds to the root part of the URL you are using to access vCloud web interface with `/api` appended.
...
If you need to configure multiple VDC, you will have to instantiate multiple connectors.
...
In a default installation the URL will be `https://ip_or_hostname/downloads/vcloudclient.tgz` where `ip_or_hostname` corresponds to the IP or the hostname of your SlipStream Server.


**Image Id of the Orchestrator**

For vCloud an image id corresponds to the (unique) name of a vApp with only one VM inside.
...
The Orchestrator doesn't need a big amount of resources so 1 CPU and 1 GB of RAM should be enough - e.g. `(1,1)`.
